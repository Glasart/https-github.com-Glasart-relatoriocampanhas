...
export const campanhasCols = [
  { key: 'data_inicio', label: 'Data Início', type: 'date' },
  { key: 'data_fim', label: 'Data Fim', type: 'date' },
  {
    key: 'plataforma_canal',
    label: 'Plataforma',
    type: 'select',
    options: ['Google Ads', 'Facebook', 'Instagram', 'Outro'],
  },
  { key: 'nome_produto', label: 'Produto', type: 'text' },
  { key: 'nome_campanha', label: 'Campanha', type: 'text' },
  { key: 'publico', label: 'Público', type: 'text' },
  { key: 'investimento', label: 'Invest. (R$)', type: 'number' },
  { key: 'impressoes', label: 'Impressões', type: 'number' },
  { key: 'alcance', label: 'Alcance', type: 'number' },
  { key: 'cliques_base_ads', label: 'Cliques Ads', type: 'number' },
  { key: 'cliques_base_rd', label: 'Cliques RD', type: 'number' },
  { key: 'ctr', label: 'CTR', type: 'readonly', isPct: true, target: 1 },
  { key: 'dif_cliques_base_rd', label: 'Dif. Cliques', type: 'readonly' },
  { key: 'leads_base_planilhas_vendas', label: 'Leads Planilha', type: 'number' },
  { key: 'leads_base_rd', label: 'Leads RD', type: 'number' },
  { key: 'cvl', label: 'CVL', type: 'readonly', isPct: true, target: 5 },
  { key: 'dif_leads_base_rd', label: 'Dif. Leads', type: 'readonly' },
  { key: 'orcamentos_semana', label: 'Orçamentos', type: 'number' },
  { key: 'pedidos_semana', label: 'Pedidos', type: 'number' },
  { key: 'leads_orcamento', label: 'Leads/Orç.', type: 'readonly', isPct: true, target: 10 },
  { key: 'orcamento_pedido', label: 'Orç./Ped.', type: 'readonly', isPct: true, target: 20 },
]
...
export function CampanhasEditableTable({
  data,
  onUpdate,
  onDelete,
  selectedIds,
  ...
  return (
    <div className="overflow-x-auto border rounded-md shadow-sm max-h-[650px] overflow-y-auto bg-white print:overflow-visible print:max-h-none print:shadow-none print:border-none custom-scrollbar">
      <Table className="text-xs whitespace-nowrap min-w-[2600px] print:min-w-full">
        ...
        <TableBody>
          ...
          {data.length === 0 && (
            <TableRow>
              <TableCell
                colSpan={campanhasCols.length + 3}
