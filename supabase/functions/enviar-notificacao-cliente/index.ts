...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: pedido, error: errP } = await supabaseClient.from('pedidos').select('*, clientes(*)').eq('id', pedido_id).single()
    if (errP || !pedido) throw new Error('Pedido não encontrado')

    // Mock API requests due to missing valid external credentials
    console.log(`Sending notification to ${pedido.clientes.email} / ${pedido.clientes.whatsapp} via ${tipo_notificacao}`)
    ...
    return new Response(JSON.stringify({ success: true, message: 'Notificação enviada com sucesso', timestamp: new Date().toISOString() }), {
